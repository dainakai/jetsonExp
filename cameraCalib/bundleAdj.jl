...
function getVector(corArray, intrSize = 128)
    ...
    if (corArray[y0+1,x0]-2.0*corArray[y0,x0]+corArray[y0-1,x0] == 0.0) || (corArray[y0,x0+1]-2.0*corArray[y0,x0]+corArray[y0,x0+1] == 0.0)
        corArray[y0,x0] += 0.00001
    end

    besty = y0 - (corArray[y0+1,x0] - corArray[y0-1,x0])/(corArray[y0+1,x0]-2.0*corArray[y0,x0]+corArray[y0-1,x0])/2.0 - intrSize/2.0 + 0.5
    # println(y0-64,"\t",besty)
    bestx = x0 - (corArray[y0,x0+1] - corArray[y0,x0-1])/(corArray[y0,x0+1]-2.0*corArray[y0,x0]+corArray[y0,x0-1])/2.0 - intrSize/2.0 + 0.5
...
"""
画像配列 `image1` と `image2` のPIVマップを計算し、返します。
"""
function getPIVMap(image1, image2, imgLen = 1024, gridSize = 128, intrSize = 128, srchSize = 256)
    gridNum = div(imgLen, gridSize)
    corArray = Array{Float64}(undef,intrSize,intrSize)
    vecArray = Array{Float64}(undef,gridNum-1,gridNum-1,2)

    for gridIdxy in 1:gridNum-1
        for gridIdxx in 1:gridNum-1

            a1, a2 = gridIdxy*gridSize - div(intrSize,2) +1:gridIdxy*gridSize + div(intrSize,2) , gridIdxx*gridSize - div(intrSize,2) +1:gridIdxx*gridSize + div(intrSize,2)
            intrWin = image1[a1, a2]
            
            for idxy in 0:srchSize - intrSize-1
                for idxx in 0:srchSize - intrSize-1
                    b1, b2 = (gridIdxy-1)*gridSize + idxy +1:(gridIdxy-1)*gridSize + intrSize + idxy, (gridIdxx-1)*gridSize + idxx +1:(gridIdxx-1)*gridSize+ intrSize + idxx
                    srchWin = image2[b1, b2]
...
function main()
    ...
    while itr <= 100
        ...
        # display(coefa)
        errorVec, x, y= getErrorVec(vecArray,coefa)
        # display(errorVec)
        println(mean(errorVec.*errorVec))
        println()
        yacobian = getYacobian(x,y)
        # display(yacobian)
